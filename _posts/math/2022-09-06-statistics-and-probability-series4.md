...
- 실생활에서 발생되는 오차의 크기는 대부분 정규분포를 따른다
...
- 표본조사를 기반으로 하는 추축 통계 방법은 크게 추정(estimation)과 검정(test)으로 나뉜다
...
- E(X)는 sigma(X_k*p_k) 이므로 E(X)도 확률변수가 될 수 있다
...
- 하지만 반복시행의 크기 n이 커지면, 경험적 확률이 수학적 확률에 한없이 가까워진다
...
- 표본평균은 기대값 m, 분산이 sigma^2/n 인 정규분포를 근사적으로 따른다
...
- 위에서 표본평균의 평균은 m 이고, 분산은 sigma^2/n 이라고 배웠으므로, 표본평균을 모평균이라고 할 수 있다
- 이제 표본분산의 평균이 sigma^2과 같은지 확인해보자. 계산하면 (n-1/n)sigma^2이 나온다
- 그래서 비편향성을 가지지 않는다. 그래서 n/n-1 을 곱해 보정한 값을 비편향 분산이라고 하고, 보통 모분산을 (n/n-1)*S^2 으로 한다
