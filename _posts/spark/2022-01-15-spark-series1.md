...
# Cluster Mode
이번에는 스파크를 구동시키는 환경에 대해서 알아보겠습니다. 스파크는 단일 서버로 동작시키는 로컬 모드와, 클러스터 환경에서 동작시키는 클러스터 모드가 있습니다.

![](../images/../../images/spark_5.png)  

로컬 모드는 위의 클러스터 환경에 있는 구성 요소들을 모두 하나의 서버에 놓는 것(Executor는 1개)과 같기 때문에 여기서는 분산 처리를 가능하게 해주는 클러스터 모드에 대해서 조금 더 자세히 알아보겠습니다. 구성 요소는 크게 다음과 같습니다.  

- 드라이버 프로그램
- 클러스터 매니저
- 워커 노드
  
![](../images/../../images/spark_7.png){: width="70%"}
...
## 클러스터 매니저  
클러스터 매니저에는 다음과 같은 것들이 있습니다.  
- __Standalone__: a simple cluster manager included with Spark that makes it easy to set up a cluster
- __YARN__: the resource manager in Hadoop 2
- __Kubernetes__: an open-source system for automating deployment, scaling, and management of containerized applications
